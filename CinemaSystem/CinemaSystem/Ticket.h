...
public:
	Ticket() = default;
	Ticket(int movieId, int row, int col, double price, const myString& date, const myString& time)
		: movieId(movieId), row(row), col(col), price(price), date(date), time(time) {
		passed = false; 
    }
